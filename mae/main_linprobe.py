...
def collect_features(model, loader):
    ...
    print(f"\ntotal time: {datetime.timedelta(seconds=time.time()-start_time)}")
    return X, Y
...
def get_args_parser():
    ...
    parser.add_argument(
        "--collect_features",
        action="store_true",
        help="collect and cache features to accelerate linear probing",
    )
    parser.add_argument(
        "--linprob_dataset",
        type=str,
        help="dataset used for linear probing. e.g. imagenet1k or imagenet100",
    )
...
def main(args):
    ...
    if args.model_type == "mae":
        ...
        if args.model_type == "clip":
            # ckpt = "/scratch/choi/model/clip-vit-base-patch16"
            ckpt = "/scratch/choi/model/CLIP-ViT-H-14-laion2B-s32B-b79K"
        elif args.model_type.startswith("finetuned-clip"):
            # ckpt = (
            #     "/scratch/choi/output/Diff-Rep/ViT-H/ip-adapter-default/checkpoint-2000"
            # )
            ckpt = args.finetune
        ...
        #     torch.nn.Linear(768, args.nb_classes),
        # )
        model.head = torch.nn.Sequential(
            torch.nn.BatchNorm1d(1280, affine=False, eps=1e-6),
            torch.nn.Linear(1280, args.nb_classes),
        )

        # hook to execute after CLIPViT forward pass
    ...
    if args.collect_features:
        # new_model = torch.nn.Sequential(*(list(list(model.children())[0].children())))[
        #     :-1
        # ]
        # print(list(model_without_ddp.children()))
        new_model = torch.nn.Sequential(*(list(model_without_ddp.children())[:-1]))
        # new_model = torch.nn.parallel.DistributedDataParallel(
        #     new_model, device_ids=[args.gpu]
        # )

        # print(new_model)
        # X_train, Y_train = collect_features(new_model, data_loader_train, device, args)
        # X_val, Y_val = collect_features(new_model, data_loader_val, device, args)

        data_loader_train = idist.auto_dataloader(
        ...
        data_loader_val = idist.auto_dataloader(
            dataset=dataset_val,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=True,
